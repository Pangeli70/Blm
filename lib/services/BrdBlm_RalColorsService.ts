...
import {
    BrdBlm_IRalColorDef
} from "../interfaces/BrdBlm_IRalColorDef.ts";

/**
 * Gestore dei colori RAL
 */
export class BrdBlm_RalColorsService {

    private static _isInited = false;
    private static _colors: Map<string, BrdBlm_IRalColorDef> = new Map();
...
RAL 9023,120-123-122,#787B7A,2-0-1-52,33.87,Pearl dark grey,Perldunkelgrau,Gris fonçé nacré,Gris oscuro perlado,Grigio scuro perlato,Parelmoerdonkergrijs`
        ...
        for (const row of rows) {
            ...
            const color: BrdBlm_IRalColorDef = {
                code: fragments[0],
